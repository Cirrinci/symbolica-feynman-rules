...
deltas on the colour indices $i,j,k,l$. Thius is a manipulation used
...
In ordere to have the lagrangian gauge invariant we need to restore the symmetry, and this can be done by acting with a covariant derivative. To restore the symmetry, we introduce a gauge field $A_\mu^a$ and so that we can replace the ordinary derivative with
\begin{equation}
...
the one in \Cref{eq_cov}. In the implementation, expanding the
...
exercise in identifying which ones\supervisor{which ones are used in this work.}. It is therefore worth collecting in one
