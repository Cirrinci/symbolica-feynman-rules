...
Two Lagrangians that differ by a total derivative describe the same physics.
...
same normal form are equivalent up to boundary terms.

\begin{minted}{python}
# Check that  phi^2 Box phi  ~  -2 phi (d_mu phi)^2  modulo total derivatives
...
This first version of the integration-by-parts layer handles unlabelled
scalar species only: it symmetrises identical scalar slots and moves
derivatives into a left-normal form. Terms containing Dirac bilinears or
gauge indices are outside its scope, so it cannot yet be used to reduce a
Green basis to the Warsaw basis. \Cref{sec:conclusion-future} returns to
this.
\end{tcolorbox}
...
The natural next step after the infinitesimal gauge transformation is to
consider the BRST transformation. The constructor
...
Everything described so far is generic machinery. Three complete models are
shipped with the toolkit, built entirely from that machinery, and they are
the subjects of the validation in \Cref{ch:validation}. Each lives in its own
directory under \path{models/}, together with its \textsc{FeynRules}
reference data, its comparison adapter and its tests.
...
\noindent The builder follows the pipeline of this chapter exactly: declare
the gauge-basis fields ($Q_L$, $L_L$, $\Phi$, $B$, $W^I$, $G$ and the
singlets), assemble the gauge, fermion, Higgs, Yukawa and ghost sectors,
compile the covariant derivatives and field strengths, expand the weak
indices into components, and finally apply the eighteen field
transformations of \Cref{sec:feynpy-transformations} in a single simultaneous
pass. It returns a \py{StandardModel} record exposing the source model, the
transformation list, the compiled physical-basis Lagrangian and the field and
...
\noindent This is the largest model, and the one that motivated most of the
engine's generality. It declares $298$ Wilson coefficients as indexed
\py{Parameter}s and $32$ Lagrangian blocks in the baryon-number-preserving
Green basis of \Cref{sec:theory-green-basis}, including dual field strengths,
$\sigma^{\mu\nu}$ structures, weak $\varepsilon$ tensors and the explicit
charge-conjugation matrices of the evanescent sector. The Standard Model core
is taken from the unbroken background-field model above, minus its
background-field ghost and gauge-fixing sectors.

Two conventions are worth noting because \Cref{ch:validation} depends on
them. \texttt{Ltot} contains the EFT contribution alone, matching the
\textsc{FeynRules} convention for the reference export, with the combined
theory available separately as \texttt{Lfull}. And the model is written in terms
of chiral fields --- \py{lL}, \py{qL}, \py{eR}, \py{uR}, \py{dR} --- so
chirality is carried by the field names rather than by explicit projectors.
