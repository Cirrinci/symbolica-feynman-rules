...
Figure~\ref{fig:pipeline-overview}.\footnote{Each TikZ figure in this chapter has a plain-text counterpart kept as a Mermaid source diagram under \texttt{viz/mermaid/}.} This shows the stages that lead from a declared \py{Model} to a Feynman-rule \py{Expression}. We can summarise it in four layers, see (Table~\ref{tab:internals-layers}).
...
Declaration &
\makecell[l]{\texttt{feynpy} (\texttt{metadata},\\
\texttt{declared}, \texttt{core})} &
...
\makecell[l]{\texttt{feynpy.lowering},\\
\texttt{lagrangian}, \texttt{compiler}} &
...
I * Quark.bar * Gamma(mu) * DC(Quark, mu)
\end{minted}
Lowering checks that \py{mu} is used consistently as a Lorentz index and that the implied spinor and colour structure is well-formed, and classifies the term as a \py{covariant_core}. This still is not a final local interaction term, because \py{DC} must first be expanded using the model's gauge data. The compiler then performs that expansion, producing ordinary local monomials such as the kinetic piece and the quark--gauge-boson vertex, and those local monomials are what finally become compiled \py{InteractionTerm}s. 
...
\makecell[l]{\texttt{interaction}} &
a local \py{InteractionTerm}; no compilation &
\py{lam * phi**4} \\

\makecell[l]{\texttt{covariant\_}\\\texttt{core}} &
canonical kinetic $\bar\psi\, i\gamma^\mu D_\mu \psi$ or $(D_\mu\Phi)^\dagger(D^\mu\Phi)$ &
quark kinetic term \\

\makecell[l]{\texttt{generic\_covariant\_}\\\texttt{monomial}} &
any other monomial that still contains \py{DC} or another covariant operator needing expansion &
current$\,\times\,$current \\

\makecell[l]{\texttt{gauge\_fixing}} &
a gauge-fixing declaration &
\py{GaugeFixing(SU3, xi)} \\

\makecell[l]{\texttt{ghost}} &
a ghost declaration &
\py{GhostLagrangian(SU3)} \\

\makecell[l]{\texttt{field\_strength\_}\\\texttt{monomial}} &
a monomial built from \py{FS(...)} factors &
$-\tfrac14 F^a F^a$ \\
...
\begin{tikzpicture}[
    ...
    ir/.style={box, fill=green!12, text width=2.8cm},
    arr/.style={-{Latex[length=2mm]}, thick}
]

\node[comp] (c1) {\makecell[c]{\texttt{covariant\_}\\\texttt{core}}};
\node[comp, below=of c1] (c2) {\makecell[c]{\texttt{generic\_covariant\_}\\\texttt{monomial}}};
\node[comp, below=of c2] (c3) {\makecell[c]{\texttt{field\_strength\_}\\\texttt{monomial}}};
\node[comp, below=of c3] (c4) {\makecell[c]{\texttt{gauge\_}\\\texttt{fixing}}};
\node[comp, below=of c4] (c5) {\makecell[c]{\texttt{ghost}}};

\node[hand, right=of c1] (h1) {\makecell[c]{\texttt{\_compile\_declared\_}\\\texttt{covariant\_core}}};
\node[hand, right=of c2] (h2) {\makecell[c]{\texttt{\_compile\_generic\_declared\_}\\\texttt{covariant\_monomial}}};
\node[hand, right=of c3] (h3) {\makecell[c]{\texttt{compile\_field\_strength\_}\\\texttt{monomial}}};
\node[hand, right=of c4] (h4) {\makecell[c]{\texttt{compile\_gauge\_fixing\_}\\\texttt{term}}};
\node[hand, right=of c5] (h5) {\makecell[c]{\texttt{compile\_ghost\_}\\\texttt{term}}};

\node[ir, right=of h3] (ir) {\makecell[c]{\texttt{InteractionTerm}\\tuple}};
...
The compiler essentially performs two operations. First, it expands each structured gauge operator into simple local monomials. Only after the complete expansion are these monomials fed back into the normal lowering process, generating a \py{InteractionTerm} (Section~\ref{sec:internals-lowering}).

This is a deliberate design choice: it prioritises generality and a single, uniform compilation pipeline over efficiency. For example, dedicated routines could be written for structures such as
$F^a_{\mu\nu}F^{a,\mu\nu}$,
$f^{abc}F^a_{\mu}{}^{\nu}F^b_{\nu}{}^{\rho}F^c_{\rho}{}^{\mu}$,
or contractions involving four field-strength tensors
\supervisor{[DONE]Here do you mean $F^2 = F_{\mu\nu}F^{\mu\nu}$ and similarly for $F^3$,$F^4$?}.
Similar dedicated routines could also be introduced for each covariant structure separately. This would make the expansion faster and, more importantly, closer to the canonical forms already familiar from textbooks. Here, however, there is no separate hard-coded compiler for these particular contractions, or for every possible power of a covariant derivative. Instead, all combinations are generated automatically through standard distribution and then passed to the same lowering phase.

The cost is that the intermediate output is much more verbose than textbook notation. For example, the non-abelian contraction
$F^a_{\mu\nu}F^{a,\mu\nu}$ expands into nine local monomials before canonicalisation combines equivalent terms again (Section~\ref{sec:internals-postprocessing}). More generally, since each non-abelian field strength contains three terms, a product of $n$ field-strength tensors initially generates $3^n$ branches. Much of the effort has therefore gone into the canonicalisation process.
...
The method \py{to_vertex_kwargs(external_legs)} performs the last model-aware step: it strips away all model-layer vocabulary and reduces one term plus a chosen set of external legs to flat, parallel lists. After which only Symbolica and Spenso objects remain.
...
There is a second rewriting that lives at this layer. A \py{FieldTransformation} (Section~\ref{sec:feynpy-transformations}) is applied not to the declared source terms but to the compiled ones: each \py{InteractionTerm} is visited, every \py{FieldOccurrence} matching a rule is replaced by the corresponding expression, and the term is rebuilt.

Placing the stage here, rather than earlier, is a deliberate choice with two consequences. First, \py{DC} and \py{FS} have already been expanded using the gauge-group metadata of the original basis, which is the only basis in which that metadata is meaningful, in fact $B_\mu$ has a hypercharge, the photon does not. Second, because the transformation operates on an \py{InteractionTerm}, it inherits the structure that the term carries: derivatives recorded in \py{derivatives} are distributed over a replacement product by the Leibniz rule automatically, conjugated occurrences receive the conjugated replacement, and the ordering that fixes fermion signs is preserved. A rule that maps one field to a sum simply multiplies the number of terms, and canonicalisation collects them again afterwards.

In the Standard Model build, the transformation stage is run with all rules acting simultaneously, so that a field introduced by one rule is not consumed by another in the same pass. The result is an ordinary \py{CompiledLagrangian}, indistinguishable in type from the one that entered, and vertex extraction proceeds from it unchanged.
...
\subsection{Fermion signs and the closed-bilinear compatibility exception}
...
\end{equation}
It has a \emph{direct} contraction, where each current stays intact, and a \emph{crossed} contraction, where the two currents exchange one identical fermion line. In the ordered-Grassmann convention, the one used by FeynPy, the exchange is odd, so the result is
\[
...
\text{direct}+\text{crossed}.
\]
 FeynRules effectively treats the two closed bilinears as bosonic composite factors before resolving the identical-fermion Wick contractions. Doing so loses the minus sign from exchanging the elementary fermion fields.\supervisor{[DONE]what do you mean here by correct'? I think that this is a case where Feynrules is just wrong.}
...
\label{sec:internals-development-support}

OpenAI Codex was used during the development of \textsc{FeynPy} as a programming assistant, mainly to support code navigation, refactoring, debugging, test writing, and the exploration of alternative implementations.
The physics conventions, algorithmic choices, validation strategy, and interpretation of the results remained the responsibility of the author, and the resulting code was reviewed as part of the development process.
% --------------------------------------------------------------
\section{Summary}
