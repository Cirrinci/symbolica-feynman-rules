...
% --------------------------------------------------------------
 ...
 Between \py{DeclaredLagrangian} and \py{CompiledLagrangian} there is an intermediate step. The declared Lagrangian is the user-written Lagrangian, while the compiled Lagrangian is a tuple of local \py{InteractionTerm}s. Therefore we first need to make every implicit structure explicit, validate its index labels, and decide wheter this term needs a model-dependent expansion. This process is what we call lowering.
...
In order to know what needs to be expanded and how by the compiler we want to analyze the terms. The single entry point is \py{_analyze_declared_source_term(...)}, which returns an \py{AnalyzedSourceTerm}. This object has the role of routing correctly every monomials through the compilation. If the term is already local, it already contains the final \py{InteractionTerm}; otherwise it stores the structured object that the compiler must expand. The compiler then consumes this analysed object expanding it accordingly to the gauge structure. The categories are summarised in Table~\ref{tab:analyzed-categories}.
...
Now the phase of full expansion of every operator of the lagrangian is done. The compiler stage is the first stage that needs all the information stored in the model concerning the gauge symmetries. Throught the loweing stage we analyzed the terms and now with the compilation we turn the remaining unexpanded object into a local interaction term. The main dispatch lives in \texttt{src/compiler/gauge.py}: \py{_compile_declared_lagrangian_terms(model)} walks the analysed source terms and sends each one to a specialised handler (Figure~\ref{fig:compiler-dispatch}). Throughout this stage the conventions are
\begin{equation}
...
\subsection{Expansion and Intercation Term building}
...
This is a deliberate design choice: it prioritises generality and a single, uniform compilation pipeline over efficiency. For example a dedicated routines could be written for $F^2$, $F^3$, $F^4$ and for each covariant structure separately, which would make it faster and more importantly in the canonical form we already know from textbooks. Here, however, there is no separate, hard-coded compiler for $F^2$, $F^3$, $F^4$ or for every possible power of a covariant derivative. The combinations are generated automatically via the standard distribution followed by a well-established lowering phase.

The cost is that the intermediate output is much more verbose than textbook notation. A non-abelian $F^2$ expands into nine local monomials before canonicalisation combines equivalent terms again (Section~\ref{sec:internals-postprocessing}). For higher operators this growth is rapid; for pure non-abelian $F^n$ it gives $3^n$ branches. Much of the effort has been to canonicalisation process.
...
\end{equation}
where we have  $t^a_{(3)}$ the colour generators, $t^I_{(2)}$ the weak generators and $Y_Q$ the hypercharge. So \py{DC(Q_L, mu)} expands to four pieces: the ordinary derivative plus one gauge current for each of the three groups. Naming one group, for example \py{DC(Q_L, mu, gauge_group=SU3C)}, keeps only that group's current together with the partial derivative.
...
The current gauge-fixing helper implements the linear covariant gauge term
\begin{equation}
    ...
    \left(\partial^\nu A^a_\nu\right),
\end{equation}
with the adjoint index omitted in the abelian case. It constructs the corresponding explicit local monomial, lowers it through the ordinary compilation pipeline, and tags the result as \py{gauge_fixing} so that the reporting and validation layers can recognise it. The user can naturally write the gauge fixingin manually, in which case these will be recognized and will receive the same tag.

This helper describes linear covariant gauge fixing. More general $R_\xi$ gauges in spontaneously broken theories may also contain Goldstone-boson contributions to the gauge-fixing function and therefore require additional terms.

For a non-abelian gauge group, the Faddeev--Popov ghost Lagrangian can be written as
\begin{equation}
...
The compiler lowers these as two ordinary local terms: the ghost kinetic
term and the ghost--gauge interaction. Since the ghost transforms in the
adjoint representation, its gauge action and index contractions reuse the
same representation machinery used for other fields. Its Grassmann
statistics, field ordering, and ghost number nevertheless remain
