...
In the Standard Model build, the transformation stage is run once with
\py{repeat=False}, with all rules acting simultaneously, so that a field
introduced by one rule is not consumed by another in the same pass. The result
is an ordinary \py{CompiledLagrangian}, indistinguishable in type from the one
that entered, and vertex extraction proceeds from it unchanged.
