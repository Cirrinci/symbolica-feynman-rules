...
There is a declaritive helper for the gauge fixing term which cna be accessed by \py{GaugeFixing(gauge_group, xi=...)}.This helper implements the linear covariant gauge term
\begin{equation}
...
Fermion signs is a delicate piece of physics in the engine that needed some care. From the beginning of the pipeline we needed to record the order of the fields all to arrive at this deepest point of the code knowing how to handle the final sign. The generic rule is the flat Grassmann sign of the fermion permutation. But there exists a case where it is not so simple. In fact this procedure would  give the \emph{wrong relative sign} for a product of closed Dirac bilinears: for identical-fermion current--current operators the flat rule produces $\text{direct} - \text{crossed}$, whereas the reference gives $\text{direct} + \text{crossed}$. The resolution is metadata-driven and narrow: when --- and only when --- every Dirac fermion slot is covered exactly once by the disjoint \py{closed_dirac_bilinears} pairs carried down from lowering, the engine suppresses the flat permutation sign and uses the all-plus bilinear convention. Generic open multi-fermion tensors keep the flat Grassmann sign. This is exactly the right boundary: it adopts the convention where the term genuinely is a product of closed currents, and nowhere else.
