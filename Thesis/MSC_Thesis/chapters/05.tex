...
Symbolica is not limited to scalar algebra, but what makes it particularly well-suited to this project is the fact that it is capable of converting \emph{tensor} expressions into a canonical form. Given a product of tensors with contracted (dummy) indices, \py{Expression.canonize_tensors} relabels the dummies deterministically and orders the factors, using symmetry metadata attached to each tensor head through the \py{is_symmetric}/\py{is_antisymmetric} flags on its symbol. Internally this rests on a graph-canonicalisation algorithm for (cyclic-)symmetric tensor networks~\cite{ruijl_symbolica_talk}. Two expressions that differ only by a renaming of dummy indices, by the allowed symmetry of a tensor head, or by a reordering of commuting factors are therefore reduced to the same normal form.

Despite Symbolica’s capabilities, it was not possible to apply canonicalisation directly, but an additional step was taken. What happens internally is that during the calculation the toolkit keeps tensors as typed Spenso objects. The symmetry information needed by \py{canonize_tensors}, however, is not attached to those live heads in the form Symbolica expects. This required a wrapper, called \py{canonize_spenso_tensors}, that temporarily swaps each Spenso tensor head into a plain Symbolica head, with symmetry flags attached where applicable, calls \py{canonize_tensors}, and then swaps the heads back.

This wrapper also keeps Lorentz, spinor, colour, and weak dummy indices in separate groups, so that indices of different kinds are never identified by mistake. When explicit index pools are not supplied, it can infer them from typed Spenso slots and from field metadata; explicit declarations remain authoritative. On top of this we have a function that adds all the tensor identities that we need in this physical contenxt. The helper \py{canonize_full} adds the project-specific reductions needed by the physics pipeline: metric contraction, canonical ordering of nested flat \(\partial_\mu\partial_\nu\) chains, local Jacobi reductions for products of two structure constants and the antisymmetry of \(f^{abc}\).
...
\py{TensorLibrary.hep_lib()}, which contains the standard tensors such as
