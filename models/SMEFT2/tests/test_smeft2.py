...
def test_smeft2_supported_subset_builds_and_compiles():
    ...
    assert len(lagrangian.terms) == 744
...
def test_smeft2_ltot_is_eft_only_and_lfull_keeps_sm_core():
    ...
    assert len(bundle.model.lagrangian().terms) == 744
    assert len(full_model.lagrangian().terms) == 798
...
def test_smeft2_comparison_report_uses_eft_only_basis():
    ...
    assert report["summary"]["feynpy_signature_count_3_to_6"] == 147
    assert report["summary"]["shared_head_matches"] == 51
