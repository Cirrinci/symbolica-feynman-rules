...
- the maintained generic API notebooks are:
  - `notebooks/getting_started.ipynb`
  - `notebooks/indices.ipynb`
  - `notebooks/flavor.ipynb`
  - `notebooks/field_strengths.ipynb`
  - `notebooks/nested_derivatives.ipynb`
  - `notebooks/field_transformations.ipynb`
  - `notebooks/compiled_operators.ipynb`
  - `notebooks/gauge_and_brst.ipynb`
- the main extraction path is now `model.lagrangian().feynman_rule(...)`
- the recommended public model-building API is now:
  `Model(source_term, fields=..., parameters=..., gauge_groups=...)`, with
  `lagrangian_decl=...` still available for explicit multi-term declarations
...
- the full supplied FeynRules `SM.fr` interaction-vertex export currently
  ...
  - 163/163 vertices match in
    `models/SM/tests/test_feynrules_full_sm_comparison.py`
...
- the general README was rewritten as a concise user-facing entry point rather
  than an internal implementation inventory
- the SM comparison review was condensed into a cohesive statement of model
  correspondence, symbolic method, measured agreement, and remaining scope
- generated bytecode, pytest caches, `.DS_Store` files, and empty directories
  left by the old layout were removed

### 2026-09-02: generic notebook refresh and documentation alignment

- the maintained generic notebook set was consolidated to eight focused API
  walkthroughs under `notebooks/`
- `README.md` now lists the maintained notebook names and documents a generic
  notebook smoke-check command
- notebook execution dependencies are declared through the `notebook` optional
  extra and included by the compatibility `requirements.txt`
- `notebooks/getting_started.ipynb` now demonstrates validation summaries,
  grouped vertex reports, `key_format="fields"`, and the
  `strip_externals` / `include_delta` output controls
- the generic notebooks were normalized with notebook cell IDs and refreshed
  from a clean execution pass
